 ...
 * 
 * La clase `ConferenciaService` proporciona métodos para validar roles de
 * usuario mediante
 * comunicación con un servicio externo a través de `WebClient`. Esta clase
 * permite verificar si
 * un usuario tiene un rol específico necesario para realizar acciones en el
 * contexto de conferencias.
 */
...
public class ConferenciaService implements ConsultaUsuarioInt {
    ...
    public boolean validarRol(Integer idUsuario, String rol) {
        String url = "http://keycloak:8080/api/usuarios/" + idUsuario;
